...
void safe_deceleration::calc_a_brake_emergency()
{
	...
	while(iterated_position < EBD_foot)
	{
		...
		// check whether
		if (i != track_conditions.read().end() &&
				...
				convert_trackcondition_to_brake_selector(i->type) > PNEUMATIC_BRAKE)
		{
			...
			auto insert = std::upper_bound(std::begin(active_conditions),std::end(active_conditions),
					*i,[](track_condition a, track_condition b)->bool{return a.end < b.end;});
			active_conditions.insert(insert,*i);

			//change the active conditions selector to reflect the new prohibition of a brake
			active_conditions_selector &= ~convert_trackcondition_to_brake_selector(i->type);
		...
		else
		{
			...
			active_conditions_selector |=  convert_trackcondition_to_brake_selector(front.type);
		...
		A_brake_emergency.modify_map(iterated_position,A_break_emergency_models[active_conditions_selector & special_brake_status_int]);
		calc_a_brake_safe(iterated_position,active_conditions_selector & special_brake_status_int);
	}
	}
